...
(define (multiply M V)
   (get-vector M V '()))
;(multiply '((-1 2 2) (-2 1 2) (-2 2 3)) '(3 4 5))
...
(define (get-min-index aux sum level)
        (if (= aux (sub1 level))
             sum
            (get-min-index (add1 aux) (+ sum (expt 3 aux)) level)))
(define (get-max-index aux sum level)
       (sub1 (+ (get-min-index aux sum level) (expt 3 (sub1 level)))))

(define (get-level sum level n)
     (if (<= n sum)
          level
         (get-level (+ sum (expt 3 level)) (add1 level) n)))

(define (get-list n min max result)
       (cond
             ((= min max) result)
             ((<= n (sub1 (+ min (quotient (add1 (- max min)) 3)))) (get-list n min (sub1 (+ min (quotient (add1 (- max min)) 3))) (append result '(1))))
             ((and (>= n (+ min (quotient (add1 (- max min)) 3))) (<= n (- max (quotient (add1 (- max min)) 3))))
                   (get-list n (+ min (quotient (add1 (- max min)) 3)) (- max (quotient (add1 (- max min)) 3)) (append result '(2))))
             (else (get-list n (add1 (- max (quotient (add1 (- max min)) 3))) max (append result '(3))))
           ))
(define (get-transformations n)
  (get-list n (get-min-index 0 1 (get-level 1 1 n)) (get-max-index 0 1 (get-level 1 1 n)) '()))
...
; Utilizați recursivitate pe coadă.
(define (get-vector-matrix Ts ppt result)
        (cond
             ((null? Ts) result)
             ((equal? 1 (car Ts)) (get-vector-matrix (cdr Ts) ppt (multiply T1 result)))
             ((equal? 2 (car Ts)) (get-vector-matrix (cdr Ts) ppt (multiply T2 result)))
             (else (get-vector-matrix (cdr Ts) ppt (multiply T3 result)))))

(define (apply-matrix-transformations Ts ppt)
  (get-vector-matrix Ts ppt ppt))

;(apply-matrix-transformations '(1 3 2 3) '(3 4 5)) 
; TODO
