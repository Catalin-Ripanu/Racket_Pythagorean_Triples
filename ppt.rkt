...
(define (dot-helper X Y result)
  (if (null? Y)
      result
      (dot-helper (cdr X) (cdr Y) (+ result  (* (car X) (car Y))))))
(define (dot-product-queue X Y)
  (dot-helper X Y 0))

(define (get-vector M V U)
     (if (null? M)
         U
         (get-vector (cdr M) V (append U (list (dot-product-queue (car M) V))))))
...
(letrec ((queue-func
            (λ (F)
              (λ (Q)
              (stream-append F (stream-lazy ((queue-func (stream (stream-first Q))) (apply stream-append (list (stream-rest Q)
                                                                                                          ((function T1) Q)
                                                                                                          ((function T2) Q)
                                                                                                          (stream-lazy ((function T3) Q)))))))
              )))
          (function
           (λ (matrix)
             (λ (flow)
             (stream (multiply matrix (stream->list (stream-first flow)))))))
         )
   ((queue-func empty-stream) (stream '(3 4 5)))))
...
(define (pairs G H)
  (letrec ( (flow (if (= (max (stream-first G) (stream-first H)) (stream-first H)) G H))
           (get-pair-min
             (λ (element)
               (λ (new-stream)
                 (λ (count)
       (stream-append element (stream-lazy (((get-pair-min (((get-count count) flow) (stream-rest new-stream))) (stream-rest new-stream)) (add1 count))))))))
           (get-count
             (λ (count)
               (λ (G)
                 (λ (H)
                   (if (zero? count)
                       empty-stream
                       (stream-append (stream (cons (stream-first G) (stream-first H))) (stream-lazy (((get-count (sub1 count)) (stream-rest G)) H))))))))
            )
    (if (= (max (stream-first G) (stream-first H)) (stream-first H))
     (((get-pair-min (stream (cons (stream-first G) (stream-first H)))) H) 2)
     (((get-pair-min (stream (cons (stream-first H) (stream-first G)))) G) 2)
...
(define (make-naturals k)
  (stream-cons k (make-naturals (+ k 2))))
(define gh-pairs-stream
  (let ( (G (make-naturals 1))
         (H (make-naturals 3))
        )
  (stream-filter (λ (pair) (and (= 1 (remainder (car pair) 2)) (= 1 (remainder (cdr pair) 2)) (< (car pair) (cdr pair)) (= 1 (gcd (car pair) (cdr pair))))) (pairs G H))))
...
(define ppt-stream-in-pair-order
  (stream-map (λ (pair) (list (* (car pair) (cdr pair)) (quotient (- (expt (cdr pair) 2) (expt (car pair) 2)) 2)
                              (quotient (+ (expt (cdr pair) 2) (expt (car pair) 2)) 2))) gh-pairs-stream)) 
