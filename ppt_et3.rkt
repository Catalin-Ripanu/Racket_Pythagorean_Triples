...
(define (multiply M V)
  (map (λ (line) (dot-product line V)) M))
...
(define (apply-functional-transformations Fs tuple)
  (foldl (λ (element arg)
            (if (procedure? element) (element arg) (multiply element arg)))
         tuple Fs))
...
(define (get-nth-tuple)
    (λ (n)
       (λ (element)
         (λ (tuple)
       (foldl (λ (elem arg)
            (if (procedure? elem) (apply elem arg) (multiply elem arg)))
              tuple
              (map (λ (number)
               (cond
                    ((= 1 number) (car element))
                    ((= 2 number) (cadr element))
                    (else (caddr element))
                    ))
...
(define (get-nth-ppt-from-matrix-transformations n) ((((get-nth-tuple) n) (list T1 T2 T3)) '(3 4 5)))
...
(define (get-nth-quadruple n) ((((get-nth-tuple) n) (list Q1 Q2 Q3)) '(1 1 2 3)))

(define (helper-aux g e f h) (list (* g h) (* 2 e f) (+ (((curry expt) e) 2) (((curry expt) f) 2))))
...
(define (get-nth-ppt-from-GH-quadruples n)
         (apply helper-aux (get-nth-quadruple n)))
